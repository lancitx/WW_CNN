...
# 4. WOA算法（减少迭代次数）
# -----------------------------
def whale_optimization_fast(n_whales=6, max_iter=8):
    """
    快速版WOA算法，减少迭代次数和模型创建
    """
    ...
    for i in range(n_whales):
        ...
        print(f"鲸鱼 {i + 1}: 适应度 = {fitness[i]:.4f}")
    ...
    # 迭代优化
    for t in tqdm(range(max_iter), desc="🔍 WOA优化进度"):
        a = 2 - 2 * (t / max_iter)
...
# 6. 可视化函数
# -----------------------------
def plot_results_clean(history_opt, woa_history, y_train, y_test_true, results, best_params):
    """清晰的结果可视化"""
    fig, axes = plt.subplots(2, 2, figsize=(12, 10))  # 减小图形尺寸

    # 1. WOA优化过程
    ax1 = axes[0, 0]
    ax1.plot(range(len(woa_history)), woa_history, 'b-o', linewidth=2, markersize=6)
    ax1.set_title('WOA Optimization Process', fontsize=14, fontweight='bold')
    ax1.set_xlabel('Iteration')
    ax1.set_ylabel('Fitness Value')
    ax1.grid(True, alpha=0.3)
    ax1.text(0.7, 0.8, f'Best: {min(woa_history):.4f}', transform=ax1.transAxes,
    ...
    try:
        ...
        # 降低dpi以减少内存使用
        plt.savefig('../img/woa_cnn_optimized_results.png', dpi=150, bbox_inches='tight')
        print("图表已保存到: ../img/woa_cnn_optimized_results.png")
    except Exception as e:
        print(f"保存图片失败: {e}")
        print("将直接显示图表...")
...
if __name__ == "__main__":
    print("=== 🚀 优化版 WOA-CNN 超参数搜索系统 ===\n")

    # 1. WOA超参数优化
    print("开始WOA超参数优化...")
    best_params, best_val_score, woa_history = whale_optimization_fast(n_whales=5, max_iter=6)
    ...
    # 6. 可视化
    plot_results_clean(history_final, woa_history, y_train, y_test_true, results, best_params)
